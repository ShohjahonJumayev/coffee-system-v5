...
const state = { user:null, branches:[], branchId:null, products:[], cart:[], page:'dashboard', cashShift:null }; 
...
async function loadCashShift(){
  ...
  const today=d.today||{};const isOpen=Boolean(d.open);
  ...
  $('shiftMeta').textContent=isOpen?`Открыта ${fmtDate(d.shift.opened_at)} · рабочая дата ${d.shift.business_date}`:`Рабочая дата ${d.businessDate} · для продаж откройте кассу.`;
  $('openShift').classList.toggle('hide',isOpen);$('closeShift').classList.toggle('hide',!isOpen);
  $('shiftTodaySales').textContent=money(today.total_sales)+' сум';$('shiftCashSales').textContent=money(today.cash_sales)+' сум';$('shiftCardSales').textContent=money(today.card_sales)+' сум';$('shiftOnlineSales').textContent=money(today.online_sales)+' сум';$('shiftExpenses').textContent=money(today.expenses)+' сум';$('shiftProfit').textContent=money(today.profit_after_expenses)+' сум';
  ...
  $('lastShiftInfo').textContent=d.lastClosed?`Последняя закрытая смена №${d.lastClosed.id}: ${money(d.lastClosed.closing_total_sales)} сум продаж · закрыта ${fmtDate(d.lastClosed.closed_at)}`:'Закрытых смен пока нет.';
}
...
$('closeShift').onclick=async()=>{if(!confirm('Закрыть кассу и посчитать итог текущей смены?'))return;try{const d=await api('/api/cash-shift/close',{method:'POST'});state.cashShift=null;const s=d.summary;openModal(`<div class="modal-head"><h2>Смена закрыта ✅</h2><button class="modal-close" onclick="closeModal()">×</button></div><p class="muted">Рабочая дата: ${esc(String(d.shift.business_date).slice(0,10))}</p><div class="shift-close-summary"><div><span>Продажи</span><b>${money(s.total_sales)} сум</b></div><div><span>Наличные</span><b>${money(s.cash_sales)} сум</b></div><div><span>Карта</span><b>${money(s.card_sales)} сум</b></div><div><span>Онлайн</span><b>${money(s.online_sales)} сум</b></div><div><span>Расходы</span><b>${money(s.expenses)} сум</b></div><div><span>После расходов</span><b>${money(s.profit_after_expenses)} сум</b></div></div><p><b>Наличные в кассе по этой смене: ${money(s.cash_balance_from_shift)} сум</b></p><button class="btn primary full" onclick="closeModal()">Готово</button>`);toast('Касса закрыта');await Promise.all([loadCashShift(),dashboard()])}catch(e){toast(e.message,true)}};
...
async function customerQr(){const [d,p]=await Promise.all([api('/api/customer-qr'),api('/api/payment-settings')]);$('menuQr').src=d.dataUrl;$('menuUrl').textContent=d.url;$('openMenuUrl').href=d.url;$('downloadQrPng').href=d.pngUrl;$('downloadQrSvg').href=d.svgUrl;$('copyMenuUrl').onclick=async()=>{await navigator.clipboard.writeText(d.url);toast('Ссылка скопирована')};$('paymeBadge').textContent=p.configured?'Перевод по чеку подключён':'Не подключён';$('paymeBadge').className='status '+(p.configured?'ready':'cancelled');$('paymeInfo').textContent=p.configured?`Клиенты могут перевести деньги на карту ${p.cardNumber||''} и загрузить чек. Оплата подтверждается вручную.`:'Чтобы включить оплату переводом, добавь CARD_TRANSFER_NUMBER и CARD_TRANSFER_HOLDER в Environment Variables Render.';$('paymeEndpoint').textContent=p.configured?`Получатель: ${p.cardHolder||'не указан'}`:'Онлайн-оплата по чеку выключена'}
