...
function setupBranches(){
  ...
  else {state.branchId=Number(localStorage.getItem('coffeeBranch')||state.branches[0]?.id||0); if(!state.branches.some(b=>Number(b.id)===state.branchId))state.branchId=Number(state.branches[0]?.id||0); select.value=state.branchId;select.disabled=false}
  select.onchange=()=>{state.branchId=Number(select.value);localStorage.setItem('coffeeBranch',state.branchId);loadPage(state.page)};
...
function renderProducts(){
  ...
  $('posProducts').innerHTML=arr.length?arr.map(p=>`<button class="product-card" data-product="${p.id}"><b>${esc(p.name)}</b><small>${esc(p.category)}</small><span>${money(p.price)} сум</span></button>`).join(''):'<div class="empty">Ничего не найдено</div>';
...
async function orders(){const arr=await api('/api/orders');$('ordersList').innerHTML=arr.length?arr.map(o=>`<div class="order-card ${o.status}"><div class="list-row"><div class="list-main"><b>Заказ №${o.id} · ${esc(o.customer_name)}</b><small>${esc(o.customer_phone)} · ${fmtDate(o.created_at)}</small></div><div class="row-actions"><span class="status ${o.status}">${statusNames[o.status]}</span><strong>${money(o.total)} сум</strong><button class="mini" onclick="openOrder(${o.id})">Открыть</button></div></div></div>`).join(''):'<div class="empty">Онлайн-заказов пока нет</div>';dashboard()}
$('refreshOrders').onclick=orders;
window.openOrder=async id=>{try{const o=await api('/api/orders/'+id);openModal(`<div class="modal-head"><h2>Заказ №${o.id}</h2><button class="modal-close" onclick="closeModal()">×</button></div><p><b>${esc(o.customer_name)}</b><br>${esc(o.customer_phone)}</p><div class="list">${o.items.map(x=>`<div class="list-row"><span>${esc(x.name)} × ${x.quantity}</span><b>${money(x.price*x.quantity)} сум</b></div>`).join('')}</div><h3>Итого: ${money(o.total)} сум</h3><div class="actions">${o.status==='new'?`<button class="btn primary" onclick="orderStatus(${o.id},'accepted')">Принять</button>`:''}${['new','accepted'].includes(o.status)?`<button class="btn success" onclick="orderStatus(${o.id},'ready')">Готов</button>`:''}${o.status==='ready'?`<button class="btn success" onclick="completeOrder(${o.id},'cash')">💵 Выдать за наличные</button><button class="btn primary" onclick="completeOrder(${o.id},'card')">💳 Выдать по карте</button>`:''}${!['completed','cancelled'].includes(o.status)?`<button class="btn danger" onclick="orderStatus(${o.id},'cancelled')">Отменить</button>`:''}</div>`)}catch(e){toast(e.message,true)}};
...
async function productsAdmin(){const arr=await api('/api/products/all');$('productsList').innerHTML=arr.length?arr.map(p=>`<div class="list-row"><div class="list-main"><b>${esc(p.name)}</b><small>${esc(p.category)} · ${money(p.price)} сум · ${p.active?'Активен':'Скрыт'}</small></div><div class="row-actions"><button class="mini" onclick="editProduct('${pack(p)}')">Изменить</button></div></div>`).join(''):'<div class="empty">Нет товаров</div>'}
$('addProduct').onclick=async()=>{try{await api('/api/products',{method:'POST',body:{name:$('productName').value.trim(),category:$('productCategory').value.trim()||'Кофе',price:Number($('productPrice').value)}});$('productName').value='';$('productCategory').value='';$('productPrice').value='';toast('Товар добавлен');productsAdmin()}catch(e){toast(e.message,true)}};
window.editProduct=raw=>{const p=typeof raw==='string'?unpack(raw):raw;openModal(`<div class="modal-head"><h2>Изменить товар</h2><button class="modal-close" onclick="closeModal()">×</button></div><input id="mName" class="input" value="${esc(p.name)}"><input id="mCategory" class="input" value="${esc(p.category)}"><input id="mPrice" class="input" type="number" value="${p.price}"><label class="actions"><input id="mActive" type="checkbox" ${p.active?'checked':''}> Показывать товар</label><div class="actions"><button class="btn primary" onclick="saveProduct(${p.id})">Сохранить</button></div>`)};
window.saveProduct=async id=>{try{await api('/api/products/'+id,{method:'PUT',body:{name:$('mName').value.trim(),category:$('mCategory').value.trim(),price:Number($('mPrice').value),active:$('mActive').checked}});closeModal();toast('Товар обновлён');productsAdmin()}catch(e){toast(e.message,true)}};
...
async function customerQr(){const d=await api('/api/customer-qr');$('menuQr').src=d.dataUrl;$('menuUrl').textContent=d.url;$('openMenuUrl').href=d.url;$('copyMenuUrl').onclick=async()=>{await navigator.clipboard.writeText(d.url);toast('Ссылка скопирована')}}
