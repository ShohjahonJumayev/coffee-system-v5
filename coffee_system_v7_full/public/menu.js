...
$('sendOrder').onclick=async()=>{
  ...
  const paymentMethod=document.querySelector('input[name="paymentMethod"]:checked')?.value||'receipt',deliveryType=document.querySelector('input[name="deliveryType"]:checked')?.value||'pickup',deliveryAddress=$('deliveryAddress').value.trim(),deliveryComment=$('deliveryComment').value.trim();
  ...
  try{
    ...
    const d=await get('/api/public/orders',options);$('drawer').classList.add('hide');const deliveryText=deliveryType==='delivery'?` Доставка: ${deliveryAddress}.`:' Самовывоз.';const promoText=d.order.discount_amount?` Промокод ${d.order.promo_code} применён: скидка ${money(d.order.discount_amount)} сум.`:'';
...
function setupPaymentControls(){const update=()=>{const receipt=paymentConfig.enabled&&document.querySelector('input[name="paymentMethod"]:checked')?.value==='receipt';$('receiptPaymentBox').classList.toggle('hide',!receipt)};document.querySelectorAll('input[name="paymentMethod"]').forEach(r=>r.onchange=update);update();}
