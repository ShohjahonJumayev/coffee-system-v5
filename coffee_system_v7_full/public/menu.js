...
function promoCodeValue(){return String(paymentConfig?.promoCode||FIRST_ORDER_PROMO_CODE).trim().toUpperCase()||FIRST_ORDER_PROMO_CODE}
...
function categorySticker(category=''){const c=String(category||'').toLowerCase();
...
function productSticker(name='',category=''){
  const n=String(name||'').toLowerCase(), c=String(category||'').toLowerCase();
  if(n.includes('чизкейк')||n.includes('тирамису')||n.includes('торт')) return '🍰';
  if(c.includes('десерт')) return '🍮';
  ...
  if(c.includes('кофе')) return n.includes('эспрессо')||n.includes('доппио')?'☕':n.includes('латте')?'🥛':n.includes('флэт')?'🤍':n.includes('раф')?'🌰':n.includes('мокка')||n.includes('мокко')?'🍫':'☕';
  return categorySticker(category);
}
function productImage(p){return ''}
...
const categoryHeroMap={
  'Все':{title:'Всё меню In coffee',text:'Кофе, лимонады, мохито, чай, милкшейки и другие позиции в одном премиальном меню.',sticker:'☕'},
  'Кофе':{title:'Кофейная классика',text:'Эспрессо, американо, капучино, латте, флэт уайт, раф и мокка — выбери свой идеальный вкус.',sticker:'☕'},
  'Айс кофе':{title:'Освежающий айс кофе',text:'Холодные кофейные напитки с мягким вкусом и красивой подачей.',sticker:'🧊'},
  'Айс матча':{title:'Нежная айс матча',text:'Матча в холодной подаче — ярко, стильно и свежо.',sticker:'🍵'},
  'Чай':{title:'Тёплый чайный раздел',text:'Чёрный, зелёный, жасминовый и фруктовый чай для спокойного настроения.',sticker:'🫖'},
  'Лимонады':{title:'Лимонады и яркие вкусы',text:'Классические и фруктовые лимонады для лёгкой свежести в течение дня.',sticker:'🍋'},
  'Мохито':{title:'Мохито со льдом и мятой',text:'Освежающий мохито с лаймом и мятой — идеальный выбор для жаркого дня.',sticker:'🌿'},
  'Милкшейки':{title:'Сливочные милкшейки',text:'Густые и сладкие милкшейки с мягкой текстурой и насыщенным вкусом.',sticker:'🥤'},
  'Фреши':{title:'Свежевыжатые фреши',text:'Яркие фруктовые напитки для свежести и витаминов.',sticker:'🍊'},
  'Добавки':{title:'Добавки к напиткам',text:'Сделай любимый напиток ещё интереснее с сиропом, пюре, джус-болами и другими дополнениями.',sticker:'✨'}
};
...
function currentOrderTotal(){return Math.max(0,currentSubtotal()-currentDiscount())}
function currentCoordinatesText(){if(Number.isFinite(Number(deliveryLat))&&Number.isFinite(Number(deliveryLng)))return `${Number(deliveryLat).toFixed(6)}, ${Number(deliveryLng).toFixed(6)}`;return $('deliveryCoordinates')?.value.trim()||''}
function coordinateMapLink(){const c=currentCoordinatesText();return c?`https://yandex.com/maps/?ll=${encodeURIComponent(c.split(', ').reverse().join(','))}&z=17&pt=${encodeURIComponent(c.split(', ').reverse().join(','))},pm2rdm`:''}
function markCoordinatesPrepared(){const el=$('coordsPrepared');if(!el)return;const c=currentCoordinatesText();if(c){el.textContent=`✅ ${c} добавлены в сообщение для отправки.`;el.classList.add('ready')}else{el.textContent='После определения координаты автоматически попадут в сообщение заказа.';el.classList.remove('ready')}}
function paymentMessage(orderId=''){
  const branch=branches.find(b=>Number(b.id)===Number(branchId));
  const address=$('deliveryAddress')?.value.trim()||'';
  const amount=currentOrderTotal();
  const lines=[
    'In coffee ☕',
    orderId?`Заказ №${orderId}`:'Подготовка онлайн-заказа',
    customer?.name?`Клиент: ${customer.name}`:'',
    customer?.phone?`Телефон: ${customer.phone}`:'',
    branch?.name?`Филиал: ${branch.name}`:'',
    address?`Доставка: ${address}`:'',
    currentCoordinatesText()?`Координаты: ${currentCoordinatesText()}`:'',
    coordinateMapLink()?`Карта: ${coordinateMapLink()}`:'',
    `Сумма к оплате: ${money(amount)} сум`,
    paymentConfig.cardNumber?`Карта: ${paymentConfig.cardNumber}`:'',
    'Чек оплаты прикреплён в заказе.'
  ].filter(Boolean);
  return lines.join('\n');
}
function setPaymentCopyStatus(text,kind=''){const el=$('paymentCopyStatus');if(!el)return;el.textContent=text;el.className='payment-copy-status'+(kind?' '+kind:'')}
async function copyCardNumber(){try{if(!paymentConfig.cardNumber)throw new Error('Карта ещё не настроена');await copyText(paymentConfig.cardNumber);setPaymentCopyStatus('✅ Номер карты скопирован. Теперь вставьте его в банковское приложение.','success')}catch(e){setPaymentCopyStatus(e.message,'error')}}
async function copyPaymentAmount(){try{const amount=currentOrderTotal();if(!amount)throw new Error('Добавьте товары в корзину');await copyText(String(amount));setPaymentCopyStatus(`✅ Сумма ${money(amount)} сум скопирована.`,'success')}catch(e){setPaymentCopyStatus(e.message,'error')}}
async function preparePaymentMessage(){try{const msg=paymentMessage();await copyText(msg);setPaymentCopyStatus('✅ Сообщение подготовлено и скопировано. Можно отправить нам в Telegram/WhatsApp.','success')}catch(e){setPaymentCopyStatus(e.message,'error')}}
async function sharePaymentMessage(textOverride=''){const msg=textOverride||paymentMessage();try{if(navigator.share){await navigator.share({title:'In coffee — заказ',text:msg});setPaymentCopyStatus('✅ Сообщение готово к отправке.','success')}else{await copyText(msg);setPaymentCopyStatus('✅ Сообщение скопировано. Вставьте его в Telegram/WhatsApp.','success')}}catch(e){if(e?.name!=='AbortError')setPaymentCopyStatus('Не удалось открыть отправку. Сообщение можно скопировать.','error')}}
...
function currentDiscount(){const code=promoCodeValue(),percent=promoPercentValue();return promoState.applied&&promoEligible()&&promoState.code===code?Math.round(currentSubtotal()*percent/100):0}
function syncPromoState(){const code=promoCodeValue();if(promoState.code)promoState.code=normalizePromoCode(promoState.code);if(!promoEligible()||promoState.code!==code)promoState.applied=false;const input=$('promoCode');if(input&&promoState.code&&input.value!==promoState.code)input.value=promoState.code}
...
function applyPromoCode(){const validCode=promoCodeValue(),percent=promoPercentValue(),code=normalizePromoCode($('promoCode').value);promoState.code=code;promoState.applied=false;if(!code){setPromoStatus('Введите промокод для скидки на первый заказ.','info');renderCart();return}if(code!==validCode){setPromoStatus(`Такого промокода нет. Используйте ${validCode}.`,'error');renderCart();return}if(!customer){setPromoStatus(`Сначала войдите в аккаунт клиента, чтобы использовать ${validCode}.`,'warn');renderCart();return}if(!promoEligible()){setPromoStatus(`${validCode} действует только на первый онлайн-заказ. Отменённые и отклонённые заказы не считаются использованным первым заказом.`,'warn');renderCart();return}promoState.applied=true;setPromoStatus(`Промокод ${validCode} применён: −${percent}% на первый заказ.`, 'success');renderCart();}
...
function renderCategoryHero(){const box=$('categoryHero');if(!box)return;const info=categoryHeroMap[category]||{title:category,text:'Подборка напитков этой категории.',sticker:categorySticker(category)};box.classList.remove('hide');box.innerHTML=`<div class="category-hero-copy"><span class="section-kicker">${esc(category==='Все'?'Подборка':'Категория')}</span><h3>${esc(info.title)}</h3><p>${esc(info.text)}</p></div><div class="category-hero-image sticker-panel"><span class="hero-sticker">${info.sticker||categorySticker(category)}</span></div>`}
...
function emoji(cat){return categorySticker(cat)}
function renderProducts(){const arr=category==='Все'?products:products.filter(p=>p.category===category);$('products').innerHTML=arr.length?arr.map(p=>{const tone=productToneClass(p),vol=productVolume(p.name),icon=productSticker(p.name,p.category);return `<div class="menu-item"><div class="menu-photo sticker-photo ${tone}"><span class="sticker-emoji">${icon}</span>${vol?`<span class="photo-volume">${vol}</span>`:''}</div><h3>${esc(p.name)}</h3><small>${esc(p.category)}</small><footer><b>${money(p.price)} сум</b><button data-add="${p.id}">+</button></footer></div>`}).join(''):'<div class="empty">Нет товаров</div>';document.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>add(Number(b.dataset.add)))}
...
$('transferCardNumber').onclick=copyCardNumber;$('copyCardBtn').onclick=copyCardNumber;$('copyAmountBtn').onclick=copyPaymentAmount;$('preparePaymentMessage').onclick=preparePaymentMessage;$('sharePaymentMessage').onclick=()=>sharePaymentMessage();
$('sendOrder').onclick=async()=>{
  ...
  try{
    ...
    lastOrderShareText=[`In coffee ☕`,`Заказ №${d.order.id}`,customer?.name?`Клиент: ${customer.name}`:'',customer?.phone?`Телефон: ${customer.phone}`:'',`Доставка: ${deliveryAddress}`,currentCoordinatesText()?`Координаты: ${currentCoordinatesText()}`:'',coordinateMapLink()?`Карта: ${coordinateMapLink()}`:'',`Сумма: ${money(d.order.total)} сум`,`Оплата: перевод на карту`,`Чек оплаты загружен и отправлен на проверку.`].filter(Boolean).join('\n');
...
function setupAccountControls(){
  ...
  $('authModal').onclick=e=>{if(e.target===$('authModal'))closeModal('authModal')};$('profileModal').onclick=e=>{if(e.target===$('profileModal'))closeModal('profileModal')};$('successCopyMessage').onclick=async()=>{try{await copyText(lastOrderShareText||'In coffee — заказ отправлен');$('successCopyMessage').textContent='✅ Сообщение скопировано'}catch{}};$('successShareMessage').onclick=()=>sharePaymentMessage(lastOrderShareText);$('successDone').onclick=()=>{$('success').classList.add('hide');$('successCopyMessage').textContent='📋 Скопировать сообщение';if(customer)openProfile()}
}
...
function setupDeliveryControls(){$('deliveryBox').classList.remove('hide');$('findAddress').onclick=findAddressOnMap;$('myLocation').onclick=useMyLocation;$('copyCoordinates').onclick=async()=>{const value=$('deliveryCoordinates').value.trim();if(!value)return alert('Сначала определите местоположение');try{await copyText(value);$('mapHint').textContent='Координаты скопированы: '+value}catch{alert(value)}};$('shareCoordinates').onclick=()=>{if(!currentCoordinatesText())return alert('Сначала нажмите «Моё местоположение»');sharePaymentMessage(paymentMessage())};markCoordinatesPrepared();if(yandexReady&&!deliveryMap)initDeliveryMap()}
...
async function setDeliveryPoint(coords,reverse=false){deliveryLat=Number(coords[0]);deliveryLng=Number(coords[1]);const coordText=`${deliveryLat.toFixed(6)}, ${deliveryLng.toFixed(6)}`;$('deliveryCoordinates').value=coordText;if(!$('deliveryAddress').value.trim()||$('deliveryAddress').value.startsWith('Координаты:'))$('deliveryAddress').value=`Координаты: ${coordText}`;if(!deliveryMap)initDeliveryMap(coords,16);if(deliveryPlacemark)deliveryPlacemark.geometry.setCoordinates(coords);else{deliveryPlacemark=new ymaps.Placemark(coords,{hintContent:'Точка доставки'},{draggable:true,preset:'islands#redDotIcon'});deliveryPlacemark.events.add('dragend',()=>setDeliveryPoint(deliveryPlacemark.geometry.getCoordinates(),true));deliveryMap.geoObjects.add(deliveryPlacemark)}deliveryMap.setCenter(coords,16,{checkZoomRange:true});$('mapHint').textContent=`Местоположение определено: ${coordText}`;markCoordinatesPrepared();setPaymentCopyStatus(`📍 Координаты ${coordText} автоматически добавлены в сообщение заказа.`,'success');if(reverse&&window.ymaps){try{const r=await ymaps.geocode(coords,{results:1}),first=r.geoObjects.get(0),line=first?.getAddressLine?.()||first?.properties?.get('text');if(line)$('deliveryAddress').value=line}catch{}}}
...
function useMyLocation(){if(!navigator.geolocation)return alert('Геолокация не поддерживается этим браузером');$('mapHint').textContent='Определяем местоположение…';navigator.geolocation.getCurrentPosition(async pos=>{const coords=[pos.coords.latitude,pos.coords.longitude];const coordText=`${Number(coords[0]).toFixed(6)}, ${Number(coords[1]).toFixed(6)}`;if(yandexReady){if(!deliveryMap)initDeliveryMap(coords,16);await setDeliveryPoint(coords,true)}else{deliveryLat=Number(coords[0]);deliveryLng=Number(coords[1]);$('deliveryCoordinates').value=coordText;if(!$('deliveryAddress').value.trim()||$('deliveryAddress').value.startsWith('Координаты:'))$('deliveryAddress').value=`Координаты: ${coordText}`;$('mapHint').textContent=`Местоположение сохранено: ${coordText}`;markCoordinatesPrepared();setPaymentCopyStatus(`📍 Координаты ${coordText} автоматически добавлены в сообщение заказа.`,'success')}},err=>{const msg=err?.code===1?'Разрешите доступ к геолокации в браузере.':'Не удалось получить местоположение.';alert(msg);$('mapHint').textContent=msg},{enableHighAccuracy:true,timeout:12000,maximumAge:30000})}
